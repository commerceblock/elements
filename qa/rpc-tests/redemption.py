...
# If whitelisting fail, redemption test fail too, because the struct of
# redemption test is copie past of whitelisting.py, but with spetifique
# modification for test redemption process
...
class RedemptionTest (BitcoinTestFramework):
    def __init__(self):
        ...
        self.num_nodes = 4
        self.extra_args = [
            ['-usehd={:d}'.format(i % 2 == 0), '-keypool=100'] for i in range(self.num_nodes)]
    ...
    def add_keys_to_whitelist(self, from_node, whitelist_node):
        ...
        for i in range(len(addresses)):
            ...
            whitelist_node.addtofreezelist(addresses[i])
            whitelist_node.addtoburnlist(addresses[i])
    ...
    def setup_network(self, split=False):
        self.nodes = start_nodes(
            self.num_nodes, self.options.tmpdir, self.extra_args[:self.num_nodes])
    ...
    def run_test(self):
        ...
        #=======================================================================
        #
        # Generate Transaction Null, 2dZRkPX3hrPtuBrmMkbGtxTxsuYYgAaFrXZ == Null
        #
        #=======================================================================
        addrNull = "2dZRkPX3hrPtuBrmMkbGtxTxsuYYgAaFrXZ"
        txid1 = self.nodes[0].sendtoaddress(addrNull, 11)
        ...
        assert_equal(self.nodes[0].querywhitelist(
            self.nodes[2].getnewaddress()), True)
