...
bool CECIES::InitEncryptor(const CPubKey& pubKey, const CKey& privKey){
...
bool CECIES::InitDecryptor(const uCharVec& encryptedMessage, const CKey& privKey, uCharVec& ciphertext){
	if(_decryptor) delete _decryptor;
	auto it1=encryptedMessage.begin()+_magic.size();
	auto it2=it1+33;
	...
	CPubKey ephemeralPub(encryptedMessage.begin()+_magic.size(), 
		encryptedMessage.begin()+_magic.size()+33);
	check(privKey, ephemeralPub);
	if(!_bOK) return false;
	uint256 ecdh_key = privKey.ECDH(ephemeralPub);
	
	it1=it2;
	it2=encryptedMessage.end()-CSHA256::OUTPUT_SIZE;
	...
	uCharVec MAC_written(it2, encryptedMessage.end());
	//Generate MAC
	uCharVec payload(encryptedMessage.begin(), it2);
...
bool CECIES::Encrypt(uCharVec& em, 
	...
	if(!InitEncryptor(pubKey, privKey)) return false;
...
bool CECIES::Decrypt(uCharVec& m, 
	...
	uCharVec ciphertext;
	if(!InitDecryptor(decoded, privKey, ciphertext)) return false;
	int paddedSize = ciphertext.size();
	m.resize(paddedSize);
	int size=_decryptor->Decrypt(ciphertext.data(), paddedSize, m.data());
	//Remove the padding.
	m.resize(size);
    return true;
}
