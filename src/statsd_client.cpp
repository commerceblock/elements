...
StatsdClient::StatsdClient(void) {
  d = new _StatsdClientData;
  d->sock = -1;
  config(g_host, g_port, "");
  srandom(time(NULL));
}
...
int StatsdClient::send(string key, size_t value, const string& type, float sample_rate) {
  ...
  if (fequal(sample_rate, 1.0))
    ...
    snprintf(buf, sizeof(buf), "%s%s:%zd|%s|@%.2f", d->ns.c_str(), key.c_str(), value, type.c_str(), sample_rate);
  return send(buf);
}
